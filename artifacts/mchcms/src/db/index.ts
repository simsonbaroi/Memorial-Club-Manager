...
export async function validateReceiptNumber(number: string): Promise<NumberValidationResult> {
  ...
  if (!existing) {
    // Check if within any active book range
  ...
  if (existing.status === 'used') {
    return { valid: false, status: 'already_used', message: `Already used for a ${existing.usedForType}` };
  }
  if (existing.status === 'voided') {
    return { valid: false, status: 'voided', message: 'This receipt number has been voided' };
  }
  if (existing.status === 'missing') {
    return { valid: false, status: 'missing', message: 'This receipt number is marked as missing' };
  }
  return { valid: true, status: 'ok', message: 'Valid receipt number' };
}
...
export async function getNextReceiptNumberSuggestion(currentNumber: string): Promise<string | null> {
  ...
  const prefix = parts[1];
  const num = parseInt(parts[2], 10);
  const paddingMatch = currentNumber.match(/-0*(\d+)$/);
  const totalDigits = paddingMatch ? currentNumber.split('-')[1].length : 6;
  const nextFormatted = `${prefix}-${String(num + 1).padStart(totalDigits, '0')}`;
...
export async function getDashboardStats() {
  ...
  const [allIncomes, allExpenses, allDonations, pendingIncomes, pendingExpenses, events, receiptBooks, voucherBooks] = await Promise.all([
  ...
  const empContrib = sum(
    allIncomes
      .filter(i => i.category === 'employee_contribution' && i.date.startsWith(monthStr))
      .map(i => i.amount)
  );
  const hospContrib = sum(
    allIncomes
      .filter(i => i.category === 'hospital_contribution' && i.date.startsWith(monthStr))
      .map(i => i.amount)
  );
  ...
  const upcomingEvents = events
    .filter(e => e.date >= todayStr && e.status === 'planned')
    .slice(0, 5);
